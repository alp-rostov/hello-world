...
class Board: #объект доска.
    ...
    def __str__(self): # вывод поля на экран
        res='<table border=1 width=200>'
        res+='<tr><td align=center>0<td align=center>1<td align=center>2<td align=center>3<td align=center>4<td align=center>5<td align=center>6</tr>'
        count=1
        for i in self.field:
            res+='<tr><td align=center>'+str(count)+'<td align=center> <a href="/shot?d=1">'+'</a><td align=center><a href="/shot?d=1">'.join(i)+ '</a></tr>'
            count+=1
        res += '</table>'
...
class Game:
    ...
    def greet(self):
        return f'<h2>Приветствуем Вас в игре "МОРСОКОЙ БОЙ"!</h2>'
    def loop(self):
        num=0
        while True:
            print('-'*20)
